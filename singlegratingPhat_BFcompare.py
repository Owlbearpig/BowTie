...
frequencies, n_s, n_p, _, _ = fbf_from_tl(HIPS_MUT1_TL_RES_PATH, a=614, b=484)
plt.plot(frequencies * 10 ** -12, n_s, label='n_s')
plt.plot(frequencies * 10 ** -12, n_p, label='n_p')
export_csv({'freq': frequencies.flatten(), 'n_s': n_s.flatten(), 'n_p': n_p.flatten()},
           'singlegratingPhat_birefringence_calc.csv')
...
np.save('measured_bf_Phat_BowTie_90deg', ref_ind_90deg)
np.save('measured_bf_Phat_BowTie_0deg', ref_ind_0deg)
np.save('measured_bf_Phat_BowTie_freqs', freq_90deg)
np.save('measured_bf_Phat_BowTie', ref_ind_0deg - ref_ind_90deg)
